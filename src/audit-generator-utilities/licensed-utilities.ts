...
const renderLicensedFiles = (
  ...
  (externalFiles || [].join(...(licensedFiles || []))).forEach(
    (flName, index) => {
      htmlStr += `
    <tr>
...
const renderLicensedtResult = (
  webRenderer: IWebRenderer,
  files: Array<Uri>
) => {
  let ctr = 0;
  let licensedFiles: Array<string> = [];
  let externalFiles: Array<string> = [];
  try {
              ...
              if (urlMatch) {
                ...
                if (url && !url.startsWith("http")) {
                  ...
                  if (fs.existsSync(absolutePath)) {
                    if (
                      !isExcludedFile &&
                      containsLicense(fs.readFileSync(absolutePath).toString())
                    ) {
...
export const findLicensedFiles = async (webRenderer: IWebRenderer) => {
