...
const renderSections = (label, id, list) => {
...
const renderUnusedCodes = (webRenderer, data) => {
...
const renderError = (webRenderer, errorMsg) => {
  webRenderer.sendMessageToUI("unusedCodeContent", {
...
const getUnusedCodes = async (webRenderer) => {
  webRenderer.sendMessageToUI("unusedCodeContent", {
  ...
  try {
    runNPMCommandWithoutPlatform(
      webRenderer,
      `npx knip --reporter json --exclude dependencies --exclude files --exclude unlisted --exclude unresolved `,
      (success, resp) => {
