...
const appendContent = (id, htmlContent, onlyIfData) => {
...
function addSnippet() {
  const snippetAlias = getFieldValue("snippet_alias");
  const snippetCode = getFieldValue("snippet_code");
  const snippetDocumentation = getFieldValue("snippet_documentation");

  if (!snippetAlias || !snippetCode) {
    return;
  }
...
function aiSearchInGPT(containerId, id) {
  ...
  if (promptInput) {
    ...
    const aiProvider = document.getElementById(`${id}_ai_provider_select`);
...
const getFieldValue = (fieldKey, defaualtValue, container) => {
  let filedControl = document.getElementById(fieldKey);

  if (container) {
    let filedControl = container;
  }
...
function saveConfigurations() {
  ...
  const geminiAPIKey = getFieldValue("gemini_ai_api_key");
...
function writeUnitTests(singleRunIndex) {
  ...
  if (promptInput) {
    ...
    if (promptVal) {
      ...
      const aiProviderSelect = document.getElementById("ai_provider_select_1");
      ...
      vscode.postMessage({
        ...
        data: {
          ...
          testFileNamingConvention: testFileConventionInput
            ...
            : null,
          gptModel: gptModelSelect ? gptModelSelect.value : null,
          aiProvider: aiProviderSelect ? aiProviderSelect.value : null,
...
function addStep(htmlContent) {
  vscode.postMessage({
    command: "knowledgeCenterCommand",
    subCommand: "addStepToTopic"
  });
}

function closeAddTopic() {
  setContent("topic_page_title", "Solutions Hub");
  showElement("topic_add_btn", true, "flex");
  showElement("topic_add_cancel", false);
  showElement("topic_add_save", false);

  vscode.postMessage({
    command: "knowledgeCenterCommand",
    subCommand: "closeAddTopic"
  });
}

function addTopic() {
  setContent("topic_page_title", "Add New Topic");
  showElement("topic_add_btn", false);
  showElement("topic_add_cancel", true, "flex");
  showElement("topic_add_save", true, "flex");
...
function editTopic(topicId) {
  setContent("topic_page_title", "Edit Topic");
  showElement("topic_add_btn", false);
  showElement("topic_add_cancel", true, "flex");
  showElement("topic_add_save", true, "flex");
  ...
  vscode.postMessage({
    command: "knowledgeCenterCommand",
    subCommand: "editTopic",
    topicId
  });
}

function handleAddTopicCommandChange(event, id) {
  const { value } = event.target;
  const showFileFields = STEP_COMMAND_FOR_FILE.includes(value);
  showElement(`field_step_file_name_${id}`, showFileFields, "flex");
  showElement(`field_step_file_content_${id}`, showFileFields, "flex");
  showElement(`field_step_command_text_${id}`, !showFileFields, "flex");
}

function saveNewTopic() {
  const topicTitle = getFieldValue("topic_title");
  const topicDesc = getFieldValue("topic_description");
  const topicStepSections = document.querySelectorAll(".topic_step");

  if (!topicTitle || !topicStepSections || topicStepSections.length === 0) {
    return;
  }

  let topicSteps = [];
  topicStepSections.forEach((topic) => {
    const id = topic.getAttribute("data-step-id");

    const stepCommandType = getFieldValue(`step_command_type_${id}`, "", topic);

    const topicObj = {
      stepTitle: getFieldValue(`step_title_${id}`, "", topic),
      stepDesc: getFieldValue(`step_description_${id}`, "", topic),
      stepCommandType,
      stepCommandText: getFieldValue(`step_command_text_${id}`, "", topic),
      stepSuccessMsg: getFieldValue(`step_success_message_${id}`, "", topic),
      stepErrorMsg: getFieldValue(`step_error_message_${id}`, "", topic)
    };

    if (STEP_COMMAND_FOR_FILE.includes(stepCommandType)) {
      topicObj.stepFileName = getFieldValue(`step_file_name_${id}`, "", topic);
      topicObj.stepFileContent = getFieldValue(
    ...
    topicSteps = [...topicSteps, topicObj];
  });

  vscode.postMessage({
    command: "knowledgeCenterCommand",
    subCommand: "saveNewTopic",
    data: {
      title: topicTitle,
...
function populateAIModelDropdown(event) {
  const { value } = event.target;
  vscode.postMessage({
    command: "populateAIModelDropdown",
    provider: value
  });
}
...
window.addEventListener("message", (event) => {
  ...
  switch (message.command) {
    ...
    case "knowledgeTopicTableContent":
      setContent("knowledge_topics", data.htmlContent);
      break;

    case "knowledgeAddTopicContent":
      setContent("knowledge_topics", data.htmlContent);
      showElement("knowledge_topics_details", false);
      break;

    case "knowledgeTopicAddStep":
      appendContent("topic_steps", data.htmlContent);
      break;
    ...
    case "aiPopulateAIModelDropdown":
      setContent(`ai_model_dropdown`, data.htmlContent);
      break;
    ...
    case "newVersionContent":
      setContent("app_common_info", data.htmlContent);
      showElement("app_common_info", true, "flex");
      break;

    case "errorSnippetAdd":
      if (!data.htmlContent) {
        showElement("add_snippet_error", false);
      }

      setContent("add_snippet_error", data.htmlContent);
      showElement("add_snippet_error", true, "flex");
      break;
  }
});
